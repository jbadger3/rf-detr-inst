...
def get_spatial_shapes(model_size):
    """
    Get the spatial shapes for each model size based on config.
    
    Args:
        model_size (str): Model size ('nano', 'small', 'medium', 'base', 'large')
        
    Returns:
        list: List of spatial shapes for the model
    """
    spatial_shapes_map = {
        'nano': [[24, 24]],
        'small': [[32, 32]],
    ...
    return spatial_shapes_map[model_size]
...
def main():
    ...
    global model_size_spatial_shapes
    model_size_spatial_shapes = get_spatial_shapes(model_size)

    # Get the appropriate model class and resolution
    ModelClass = get_model_class(model_size)
    resolution = get_model_resolution(model_size)
