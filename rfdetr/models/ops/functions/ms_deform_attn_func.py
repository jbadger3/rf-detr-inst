...
def ms_deform_attn_core_pytorch_inference(value, value_spatial_shapes, sampling_locations, attention_weights):
    """"for debug and test only, need to use cuda version instead
    """
    #remove the batch dim to allow for computation on rank 5 tensors
    # n_heads, head_dim, N
    ...
    sampling_value_list = []
    for lid_, (H, W) in enumerate(value_spatial_shapes):
        #n_heads, head_dim, H, W
        value_l_ = value_list[lid_].view(n_heads, head_dim, H, W)
        # B, Len_q, n_heads, P, 2 -> B, n_heads, Len_q, P, 2 -> B*n_heads, Len_q, P, 2
